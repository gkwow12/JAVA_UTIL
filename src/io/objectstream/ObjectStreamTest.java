	...
	void saveData() {
		...
		*/
		...
		try{
			...
			Record r = new Record(name, age, height, bloodType);   //기본 생성자 없으면 오류나니까 꼭 확인
			oos.writeObject(r);
	...
	void readData() {
		...
		*/
			try{
				...
				ObjectInputStream oss = new ObjectInputStream(new FileInputStream("data2.txt"));
				//Record r = new Record(name, age, height, bloodType);
				Record r= (Record) oss.readObject();  //형변환
